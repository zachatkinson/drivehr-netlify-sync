...
/**
 * API-based job fetching strategy
 *
 * Fetches job data directly from DriveHR API endpoints using structured
 * HTTP requests. This is the preferred method as it provides the most
 * reliable and structured data format.
 *
 * The strategy attempts multiple API endpoints in order of preference,
 * handling different API versions and endpoint formats that DriveHR
 * may expose.
 *
 ...
 *   console.log(`Fetched ${jobs.length} jobs via API`);
 * }
 * ```
 * @since 1.0.0
 * @see {@link DriveHrUrlBuilder.buildApiUrls} for URL construction
 * @see {@link JobDataExtractor.extractFromApiResponse} for data extraction
 */
export class ApiJobFetchStrategy implements IJobFetchStrategy {
  public readonly name: FetchMethod = 'api';

  /**
   * Check if API strategy can handle the configuration
   *
   * Requires both company ID and API base URL to be present
   * for successful API-based job fetching.
   *
  ...
  /**
   * Fetch jobs from DriveHR API endpoints
   *
   * Attempts to fetch job data from multiple API endpoints in order of
   * preference. Each endpoint may have different response formats, so
   * the method uses flexible data extraction to handle various structures.
   *
   * @param config - DriveHR API configuration with endpoint details
   * @param httpClient - HTTP client for making API requests
   * @returns Promise resolving to array of raw job data
   * @throws {Error} When all API endpoints fail to return valid data
   * @example
   * ```typescript
   * const strategy = new ApiJobFetchStrategy();
   * try {
  ...
  public async fetchJobs(config: DriveHrApiConfig, httpClient: IHttpClient): Promise<RawJobData[]> {
    ...
    for (const url of apiUrls) {
      try {
        ...
        if (response.success) {
          const jobs = JobDataExtractor.extractFromApiResponse(response.data);
          if (JobDataExtractor.isValidJobArray(jobs)) {
            return jobs;
          }
        }
      } catch (error) {
        JobFetchErrorHandler.logAndContinue('API URL', url, error);
      }
    }
...
/**
 * JSON endpoint fetching strategy
 *
 * Fetches job data from dedicated JSON endpoints that provide structured
 * job data in a standardized format. This strategy is used when DriveHR
 * exposes job data through dedicated JSON APIs or endpoints.
 *
 * @implements {IJobFetchStrategy}
 * @example
 * ```typescript
 * const strategy = new JsonJobFetchStrategy();
 * const config = { careersUrl: 'https://example.com/careers/list' };
 *
 * if (strategy.canHandle(config)) {
 *   const jobs = await strategy.fetchJobs(config, httpClient);
 *   console.log(`Fetched ${jobs.length} jobs from JSON endpoint`);
 * }
 * ```
 * @since 1.0.0
 * @see {@link DriveHrUrlBuilder.buildCareersJsonUrl} for URL construction
 */
export class JsonJobFetchStrategy implements IJobFetchStrategy {
  public readonly name: FetchMethod = 'json';

  /**
   * Check if JSON strategy can handle the configuration
   *
   * Requires a careers URL to be present as the JSON endpoint
   * is derived from the careers page URL.
   *
   * @param config - DriveHR API configuration to validate
  ...
  /**
   * Fetch jobs from JSON endpoint
   *
   * Retrieves job data from a JSON endpoint derived from the careers URL.
   * Handles different JSON response structures and validates that the
   * response contains a valid array of job data.
   *
   * @param config - DriveHR API configuration with careers URL
   * @param httpClient - HTTP client for making the JSON request
   * @returns Promise resolving to array of raw job data
   * @throws {Error} When JSON endpoint is not accessible or returns invalid data
   * @example
   * ```typescript
   * const strategy = new JsonJobFetchStrategy();
   * try {
   *   const jobs = await strategy.fetchJobs(config, httpClient);
   *   console.log(`JSON endpoint returned ${jobs.length} jobs`);
   * } catch (error) {
   *   console.error('JSON endpoint failed:', error.message);
   * }
   * ```
   * @since 1.0.0
   */
  public async fetchJobs(config: DriveHrApiConfig, httpClient: IHttpClient): Promise<RawJobData[]> {
    ...
    if (!response.success) {
      throw new Error('JSON endpoint not accessible');
    }
...
/**
 * Embedded data fetching strategy for JSON-LD and embedded JavaScript
 *
 * Extracts job data from structured data embedded within HTML pages,
 * including JSON-LD structured data (schema.org JobPosting) and
 * JavaScript variables containing job data. This strategy is useful
 * when job data is embedded in the page for client-side rendering.
 *
 * @implements {IJobFetchStrategy}
 * @example
 * ```typescript
 * const strategy = new EmbeddedJobFetchStrategy();
 * const config = { careersUrl: 'https://example.com/careers' };
 *
 * if (strategy.canHandle(config)) {
 *   const jobs = await strategy.fetchJobs(config, httpClient);
 *   console.log(`Extracted ${jobs.length} jobs from embedded data`);
 * }
 * ```
 * @since 1.0.0
 * @see {@link https://schema.org/JobPosting} for JSON-LD JobPosting schema
 */
export class EmbeddedJobFetchStrategy implements IJobFetchStrategy {
  public readonly name: FetchMethod = 'json-ld';

  /**
   * Check if embedded data strategy can handle the configuration
   *
   * Requires a careers URL to be present for downloading and parsing HTML.
   *
   * @param config - DriveHR API configuration to validate
   * @returns True if careersUrl is provided
   * @since 1.0.0
  ...
  /**
   * Fetch jobs from embedded data in HTML
   *
   * Downloads the careers page and attempts to extract job data from
   * embedded structured data. Tries JSON-LD first (preferred for SEO
   * and structured data), then falls back to embedded JavaScript variables.
   *
   * @param config - DriveHR API configuration with careers URL
   * @param httpClient - HTTP client for downloading HTML content
   * @returns Promise resolving to array of raw job data
   * @throws {Error} When HTML page is not accessible or no embedded data found
   * @example
   * ```typescript
   * const strategy = new EmbeddedJobFetchStrategy();
   * try {
   ...
   *   console.log(`Extracted ${jobs.length} jobs from embedded data`);
   * } catch (error) {
   *   console.error('Embedded data extraction failed:', error.message);
   * }
   * ```
   * @since 1.0.0
   */
  public async fetchJobs(config: DriveHrApiConfig, httpClient: IHttpClient): Promise<RawJobData[]> {
    ...
    // Try JSON-LD first
    const jsonLdJobs = this.extractJsonLdJobs(response.data);
    if (jsonLdJobs.length > 0) {
      return jsonLdJobs;
    }

    // Try embedded JavaScript
    const embeddedJobs = this.extractEmbeddedJobs(response.data);
   ...
   *
   * Searches for and parses JSON-LD script tags containing JobPosting
   * structured data according to schema.org standards. This is the
   * preferred method for embedded job data as it's SEO-friendly.
   *
   * @param html - HTML content to search for JSON-LD data
   ...
   *   </script>
   * `;
   * const jobs = this.extractJsonLdJobs(html);
   * // Returns: [{ title: 'Software Engineer', description: '...' }]
   * ```
   * @since 1.0.0
   * @see {@link https://schema.org/JobPosting} for JobPosting schema
   */
  private extractJsonLdJobs(html: string): RawJobData[] {
    ...
    for (const match of jsonLdMatches) {
      try {
        ...
        if (Array.isArray(data)) {
          return data.filter(item => item['@type'] === 'JobPosting');
        }
      } catch (error) {
  ...
  /**
   * Extract job data from embedded JavaScript variables
   *
   * Searches for common JavaScript variable patterns where job data
   * might be embedded in the HTML page for client-side rendering.
   * Attempts multiple common variable names and formats.
   *
   * @param html - HTML content to search for embedded JavaScript
   * @returns Array of job data extracted from JavaScript variables
   * @example
   * ```typescript
   * const html = `
   *   <script>
   ...
   *   </script>
   * `;
   * const jobs = this.extractEmbeddedJobs(html);
   * // Returns: [{ id: '1', title: 'Developer', ... }, { id: '2', title: 'Designer', ... }]
   * ```
   * @since 1.0.0
   * @see {@link JobDataExtractor.extractFromApiResponse} for data extraction
   */
  private extractEmbeddedJobs(html: string): RawJobData[] {
    const patterns = [
      /window\.jobData\s*=\s*(\{[\s\S]*?\});/,
      /window\.careers\s*=\s*(\{[\s\S]*?\});/,
      /window\.jobs\s*=\s*(\[[\s\S]*?\]);/,
    ];
    ...
    for (const pattern of patterns) {
      ...
      if (match?.[1]) {
        try {
          ...
          const jobs = JobDataExtractor.extractFromApiResponse(data);
          if (JobDataExtractor.isValidJobArray(jobs)) {
            return jobs;
          }
        } catch (error) {
          JobFetchErrorHandler.logAndContinue('Embedded JS parsing', 'window data', error);
        }
      }
    }
...
export class JobFetchService {
  ...
  constructor(
    ...
    this.strategies = [
      ...
      new JsonJobFetchStrategy(),
      new HtmlJobFetchStrategy(htmlParser),
      new EmbeddedJobFetchStrategy(),
    ] as const;
  }
